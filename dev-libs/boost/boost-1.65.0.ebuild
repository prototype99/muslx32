...
PATCHES=(
	...
	"${FILESDIR}/${PN}-1.55.0-flags-mx32.patch"
)
...
create_user-config.jam() {
	...
	#	if [[ "${ABI}" == "amd64" ]] ; then
	#		append-cflags ${CFLAGS2_amd64}
	#		append-cxxflags ${CFLAGS2_amd64}
	#	elif [[ "${ABI}" == "x86" ]] ; then
	#		append-cflags ${CFLAGS2_x86}
	#		append-cxxflags ${CFLAGS2_x86}
	#	elif [[ "${ABI}" == "x32" || "${ABI}" == "default" ]] ; then
	#		append-cflags ${CFLAGS2_x32}
	#		append-cxxflags ${CFLAGS2_x32}
	#	fi
	#fi
