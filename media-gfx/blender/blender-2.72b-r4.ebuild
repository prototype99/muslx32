...
src_prepare() {
	...
	if [[ "${CHOST}" =~ "muslx32" ]] ; then
		sed -i -e "s|#  define HAVE_MALLOC_STATS||g" intern/guardedalloc/intern/mallocn_intern.h || die
		epatch "${FILESDIR}"/${PN}-2.72b-musl-stacktrace_generic-inl_h.patch
		epatch "${FILESDIR}"/${PN}-2.72b-musl-execinfo_h.patch
		epatch "${FILESDIR}"/${PN}-2.72b-musl-creator_c.patch
	fi
